...
class CustomBatchNormManualFunction(torch.autograd.Function):
    ...
    def forward(ctx, input, gamma, beta, eps=1e-5):
        ...
        in_mu = torch.add(input, -torch.mean(input, 0))
        theta = torch.mean(torch.pow(in_mu, 2), 0)
        rvar = torch.reciprocal(torch.sqrt(theta + eps))
        norm = torch.mul(in_mu, rvar)
        out = torch.add(torch.mul(gamma, norm), beta)
    ...
    def backward(ctx, dout):
        ...
        dgamma = torch.sum(dout * ctx.norm, 0)
        dhat = torch.mul(dout, ctx.gamma)
        dx = (1 / M) * ctx.rvar * (M * dhat - torch.sum(dhat, 0)
                                    - ctx.norm * torch.sum(dhat * ctx.norm, 0))
